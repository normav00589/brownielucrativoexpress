      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="space-y-6 md:space-y-8">
          {reviews.map((review, index) => (
            <div key={index} className="bg-card p-6 md:p-8 rounded-2xl shadow-soft hover:shadow-neon transition-all duration-300 border border-accent/20">
              <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-shrink-0">
                  <img 
                    src={review.image} 
                    alt={review.name}
                    className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover border-4 border-accent/30"
                  />
                </div>

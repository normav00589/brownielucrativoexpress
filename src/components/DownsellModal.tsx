...
export const DownsellModal = ({ isOpen, onClose }: DownsellModalProps) => {
  ...
  const progressPercent = totalSeconds / 180 * 100;
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[92vw] max-w-[420px] p-0 border-0 bg-transparent overflow-visible will-change-transform">
        ...
        <div className="max-h-[80vh] overflow-y-auto overflow-x-hidden rounded-2xl overscroll-contain touch-pan-y">
          <div className="relative bg-gradient-to-b from-background via-background to-primary/20 border-2 border-accent/60 rounded-2xl shadow-lg">
            
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 flex gap-1 text-2xl">
              🔥🔥🔥
            </div>
            ...
            <div className="p-4 pt-6">
              ...
              <div className="bg-background/90 rounded-xl p-3 mb-3 border border-accent/40">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ⏰
                  <span className="font-heading font-bold text-white text-xs">OFERTA EXPIRA EM:</span>
                </div>
              ...
              <div className="text-center mb-3 relative">
                ...
                <span className="absolute -left-2 top-0 text-accent animate-pulse">⭐</span>
                <span className="absolute -right-2 top-2 text-primary animate-pulse text-sm" style={{ animationDelay: '0.5s' }}>⭐</span>
              </div>

              <div className="grid grid-cols-1 gap-1.5 mb-3">
                {benefits.map((benefit, index) =>
                <div key={index} className="flex items-center justify-between bg-white/5 rounded-lg px-2 py-1.5 border border-white/10">
              ...
              <div className="bg-gradient-to-r from-accent/20 to-primary/20 border border-accent/40 rounded-lg p-2 mb-3 flex items-center gap-2">
                <div className="bg-accent/30 rounded-full p-1.5 text-lg">🎁</div>
